...
export default function CorrelationRanking() {
  const [rankings, setRankings] = useState<Ranking[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    fetch("/api/analysis/ranking")
      .then((res) => res.json())
  ...
  if (isLoading) {
    return (
      <div className="glass-panel p-6 rounded-3xl animate-pulse">
        <div className="h-6 bg-slate-700/50 rounded-lg w-1/3 mb-4"></div>
        <div className="h-12 bg-slate-700/30 rounded-2xl mb-2"></div>
        <div className="h-12 bg-slate-700/30 rounded-2xl"></div>
      </div>
    );
  }
  ...
  if (validRankings.length === 0) {
    return <div className="glass-panel p-6 rounded-3xl text-center text-slate-400 text-sm border-dashed">Not enough data to calculate correlations yet.</div>;
  }
